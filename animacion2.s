...
jumpCounter: .res 1
frameCounter: .res 1
timer: .res 1 ; indicates number of sprite in walking animations
pad1: .res 1 ; Button input states for Player 1
animationState: .res 1 ; indicates the animation that is being done in the sequence
timerSequence: .res 1
...
;   PHA
  ...
  ; Check the current state and timer to decide the next state
  LDA timerSequence
  CMP #120 ; 3 seconds per animation
  BNE skipChange
  ...
  ; Check if the state exceeds the number of animations
  LDA animationState
  CMP #06
  BNE skipChange
...
;   skipChange:
  ; Based on the current state, call the respective animation subroutine
    ; LDA animationState
;   CMP #00
;   BNE skip1
;   JMP walkRight
